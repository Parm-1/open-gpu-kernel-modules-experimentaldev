...
run nvidia-smi-minimal nvidia-smi --query-gpu=name,driver_version,display_active,display_mode --format=csv,noheader
run drm-links bash -c 'for p in /sys/class/drm/card*-*; do [ -e "$p" ] || continue; printf "%s\t" "$p"; readlink -f "$p/device/driver"; done'
run modetest-nvidia modetest -M nvidia-drm -c
run display-properties bash -c 'for p in /sys/class/drm/card*-*/status; do [ -e "$p" ] || continue; printf "%s: " "$p"; cat "$p"; done'
# Record only EDID presence and byte count, never EDID bytes or a persistent hash.
run edid-presence bash -c 'for p in /sys/class/drm/card*-*/edid; do [ -s "$p" ] || continue; printf "%s\t%s bytes\n" "$p" "$(wc -c <"$p")"; done'
run recent-nvidia-kernel-log bash -o pipefail -c "dmesg 2>/dev/null | grep -Ei 'nvidia|nvrm|hdcp|drm' | tail -n 1000 | sed -E 's/GPU-[0-9A-Fa-f-]+/GPU-<redacted>/g; s/([[:alnum:]_.-]+) kernel:/<host> kernel:/' || true"
...
limits nvidia-smi to non-unique operational fields and does not run drm_info,
vulkaninfo, or a broad vendor bug-report collector. Local filesystem paths,
